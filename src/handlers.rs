...
fn get_all_technologies(pool: web::Data<Pool>) -> Result<Vec<Technology>, diesel::result::Error> {
...
fn db_get_technology_by_id(pool: web::Data<Pool>, technology_id: i32) -> Result<Technology, diesel::result::Error> {
  let conn = pool.get().unwrap();
  technologies.find(technology_id).get_result::<Technology>(&conn)
}
...
fn add_single_technology(
  ...
  let res = insert_into(technologies).values(&new_technology).get_result(&conn)?;
  Ok(res)
}

fn delete_single_technology(db: web::Data<Pool>, technology_id: i32) -> Result<usize, diesel::result::Error> {
  let conn = db.get().unwrap();
